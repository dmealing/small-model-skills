...
## Steps
1. Run `find-files <query> [path]`. It searches file *names* (via fd/find) and file *contents* (via
   ripgrep/grep) under the path — defaulting to the configured `SEARCH_ROOT`, or the current directory —
   and prints matching paths plus a verdict. It is read-only.
2. Read the results:
