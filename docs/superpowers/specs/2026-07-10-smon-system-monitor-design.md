...
SMON_NOTIFY="ha-push"       # space-separated backends: ha-push kuma stdout
...
+ the first thing to check. 60s timeout, temp 0. Any failure → ship the raw verdict prose. Only
fires on the minority of sweeps with a transition, so cost ≈ zero.
